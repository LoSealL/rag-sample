...
for file_path_str in ["src/store.cpp", "src/pipeline.cpp"]:
    ...
    for u in file_entry["units"]:
        ...
        if matches:
            for m in matches:
                if m.metadata.line_start != u['start_line'] or m.metadata.line_end != u['end_line'] or m.metadata.chunk_type != u['type']:
                    print(f"  {u['name']}: ref {u['type']} {u['start_line']}-{u['end_line']}, got {m.metadata.chunk_type} {m.metadata.line_start}-{m.metadata.line_end}")
        else:
